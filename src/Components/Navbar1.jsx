...
const Navbar1 = () => {
  ...
  return (
    <header className=' z-50  top-0 left-0 relative sm:fixed w-full bg-gray-50 shadow-lg py-4'>
      <nav className="items-center justify-between px-4 mx-auto max-w-screen-xl sm:px-8 flex sm:space-x-6">
        <a href="/">
          <div className=" flex-1 flex items-center">
            <img
              ...
              alt="Float UI logo"
            />
            <h3 className=" text-gray-900 font-semibold italic text-4xl ml-1">
        ...
        <ul className="py-4 flex-1 items-center hidden sm:flex space-x-3 sm:space-x-6 sm:justify-end">
          {navigation.map((item, idx) => (
            <li
              className={`${pathname == item.path ? "text-indigo-500":"text-gray-700"} hover:text-indigo-500 font-semibold transition-all duration-500 `}
              key={idx}
            >
        ...
        <div className=" sm:hidden flex items-center">
          <button onClick={()=> setOpen(true)} className=" border border-indigo-400/90 p-1 rounded-md">
            <Bars3BottomRightIcon className=" fill-indigo-400/90 h-9 w-9" />
          </button>
        </div>
      </nav>
